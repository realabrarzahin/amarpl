...
static void adjust_capacity(Table *table, size_t capacity) {
  ...
  for (size_t i = 0; i < table->capacity; i++) {
    ...
    Entry *new_destination = find_entry(entries, capacity, entry->key);

    new_destination->key = entry->key;
    new_destination->value = entry->value;
...
void mark_table(Table *table) {
  ...
  for (size_t i = 0; i < table->capacity; i++) {
    Entry *entry = &table->entries[table->capacity];
